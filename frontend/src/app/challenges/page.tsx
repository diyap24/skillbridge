...
export default function ChallengesPage() {
  ...
  const { data, isLoading, error } = useQuery<{ data: Challenge[] }>({
    queryKey: ['challenges'],
    queryFn:  () => api.get('/challenges?page=1&pageSize=50').then(r => r.data),
  });
  ...
  const filtered = useMemo(() => {
    ...
    return data.data.filter(c => {
      ...
      const f = filter === 'All' || c.difficulty === filter;
      return s && f;
    });
  }, [data, search, filter]);
      ...
      <div className="relative max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row gap-3 mb-6 animate-fade-up [animation-delay:80ms]">
          <div className="relative flex-1">
            <svg className="absolute left-3.5 top-1/2 -translate-y-1/2 text-blush/30 flex-shrink-0"
            ...
            <input value={search} onChange={e => setSearch(e.target.value)}
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-deep/30 border border-royal/20
                         ...
                         focus:outline-none focus:border-mauve/50 focus:ring-2 focus:ring-mauve/10
              ...
              <button key={d} onClick={() => setFilter(d)}
                className={`px-4 py-2.5 rounded-xl text-xs font-semibold whitespace-nowrap
                             ...
                             ${filter === d
                               ...
                               : 'border-royal/20 text-blush/50 hover:border-royal/40 hover:text-blush/80 bg-transparent'}`}>
                {d}
              </button>
            ))}
          </div>
        ...
        {!isLoading && data && (
          <p className="text-[11px] text-blush/25 mb-4">
            ...
            {filtered.map((c, i) => (
              <Link key={c.id} href={`/challenges/${c.id}`}
                className="group relative rounded-2xl overflow-hidden
                           border border-royal/20 bg-deep/25 backdrop-blur-sm p-6
                           hover:border-mauve/40 hover:-translate-y-1
                           hover:shadow-[0_8px_32px_rgba(82,43,91,0.3)]
                           transition-all duration-300 animate-fade-up"
                style={{ animationDelay: `${i * 55}ms` }}>
                <div className="absolute inset-0 opacity-0 group-hover:opacity-100
                                transition-opacity duration-500 pointer-events-none
                                bg-gradient-to-br from-royal/15 via-transparent to-mauve/5" />
